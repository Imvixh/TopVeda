-- ==============================================================================
-- TopVeda Migration: Fix Admin Application Review RPC (No Phone Verification)
-- Architecture: Admin registration creates profiles.role = 'ADMIN'.
-- Access to Admin Panel requires profiles.role = 'ADMIN' AND application.status = 'APPROVED'.
-- Approval transitions application status: PENDING -> APPROVED (or REJECTED).
-- Phone verification is intentionally not required by Supabase Auth for approval.
-- ==============================================================================
...
BEGIN
    ...
    -- 7. Check applicant email is verified
    IF v_user.email_confirmed_at IS NULL THEN
        RAISE EXCEPTION 'Applicant email is not verified.';
    END IF;

    -- 8. Verify application email consistency against auth.users
    ...
    IF p_decision = 'APPROVED' THEN
        -- Atomically update application status to APPROVED
    ...
    ELSIF p_decision = 'REJECTED' THEN
        -- Atomically update application status to REJECTED with optional reason
        UPDATE public.admin_applications
        SET status = 'REJECTED',
