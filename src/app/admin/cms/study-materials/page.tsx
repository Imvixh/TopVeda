...
function StudyMaterialsCmsContent() {
  ...
  const handleToggleVisibility = async (sm: CmsStudyMaterial) => {
    const nextVal = !sm.is_visible;
    const { error } = await CmsService.toggleVisibility(supabase, "cms_study_materials", sm.id, nextVal);
    if (error) {
      setFeedback({ type: "error", message: error.message || "Failed to update visibility." });
    } else {
      setFeedback({
        type: "success",
        message: `Document "${sm.title}" visibility is now ${nextVal ? "Visible" : "Hidden"}.`,
      });
      handleManualRefresh();
    }
  };
    ...
    if (publishTarget.action === "PUBLISH") {
      try {
        ...
        if (!res.ok || result.error) {
          setFeedback({ type: "error", message: result.error || "Failed to publish study material." });
        } else {
          setFeedback({ type: "success", message: `Study material "${publishTarget.title}" published successfully.` });
          setPublishTarget(null);
          handleManualRefresh();
        }
      } catch (err: unknown) {
        const error = err instanceof Error ? err : new Error(String(err));
        setFeedback({ type: "error", message: error.message || "Failed to publish study material." });
      } finally {
        setIsPublishProcessing(false);
      }
    } else {
      // Unpublish action
      const { error } = await CmsService.unpublishContent(supabase, "STUDY_MATERIAL", publishTarget.entityId);
      setIsPublishProcessing(false);
      if (error) {
        setFeedback({ type: "error", message: error.message || "Failed to unpublish study material." });
      } else {
        setFeedback({ type: "success", message: `Study material "${publishTarget.title}" unpublished and returned to draft.` });
        setPublishTarget(null);
        handleManualRefresh();
      }
    }
  ...
  const handleOpenPreview = async (sm: CmsStudyMaterial) => {
    let signedPdf: string | null = null;
    if (sm.file_url && sm.file_url.includes("/")) {
      const { signedUrl } = await StorageService.getSignedUrl(supabase, "study-materials", sm.file_url, 3600);
      signedPdf = signedUrl;
    }
    setPreviewItem({ type: "STUDY_MATERIAL", data: sm, signedPdf });
  };
                  ...
                  return (
                    <tr key={sm.id} className="hover:bg-brand-bg-warm/40 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <ScheduleStatusBadge startsAt={sm.starts_at} endsAt={sm.ends_at} />
                      </td>
                      <td className="py-3.5 px-4 text-center">
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          type="button"
                          onClick={() => handleToggleVisibility(sm)}
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <ContentStatusBadge status={sm.status} />
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {sm.status !== "PUBLISHED" ? (
                            <Button
                              ...
                              onClick={() =>
                                setPublishTarget({
                                  ...
                                  isVisible: sm.is_visible,
                                  startsAt: sm.starts_at,
                                  endsAt: sm.ends_at,
                                  displayOrder: sm.display_order,
                                  featuredNote: sm.material_type.replace(/_/g, " ").toUpperCase(),
                                  action: "PUBLISH",
                                })
                              }
                            ...
                            <Button
                              ...
                              title="Unpublish Study Material"
                            >
